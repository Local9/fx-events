...
namespace FxEvents.Samples.Implementation.Events
{
    public class ServerGateway : BaseGateway
    {
        ...
        public ServerGateway(Server script)
        {
            Logger = new Logger();
            ...
            script.Hook(EventConstant.InvokePipeline, new Action<string, byte[]>(Invoke));
            script.Hook(EventConstant.ReplyPipeline, new Action<string, byte[]>(Reply));
        ...
        public void Push(string pipeline, ISource source, byte[] buffer)
        {
            if (source.Handle != ClientId.All.Handle)
                BaseScript.TriggerClientEvent(Server.ToPlayer(source.Handle), pipeline, buffer);
            else
        ...
        private void GetSignature([FromSource] string source)
        {
            ...
            BaseScript.TriggerClientEvent(Server.ToPlayer(client.Handle), SignaturePipeline,
                signature);
        }
        ...
        private async void Invoke([FromSource] string source, byte[] buffer)
        {
            ...
            using var context =
                new SerializationContext(EventConstant.InvokePipeline, "(Gateway) Invoke", Serialization, buffer);
            ...
            {
                API.DropPlayer(client.Handle.ToString(), $"Operation failed: {message.Endpoint} (Timed Out)");
            }
        }
        ...
        private void Reply([FromSource] string source, byte[] buffer)
        {
            ...
            using var context =
                new SerializationContext(EventConstant.ReplyPipeline, "(Gateway) Reply", Serialization, buffer);
